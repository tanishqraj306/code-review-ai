...
app.get('/api/auth/github', (req, res) => {
  ...
  // Note: Scope must be space-separated
...
app.get('/api/auth/callback', async (req, res) => {
  ...
  try {
    ...
    const userPayload = {
      ...
      accessToken, // Stored to perform actions on behalf of user
      lastLogin: new Date(),
    };
...
app.post('/api/repositories', protectRoute, async (req, res) => {
  ...
  try {
    ...
    if (existingRepo) {
      ...
      return res.status(409).send({ message: 'You have already added this repository.' });
    }
    ...
    if (!userDoc || !userDoc.accessToken) {
      return res.status(401).send({ message: 'User GitHub token not found. Please log in again.' });
    }
    ...
    try {
      ...
      if (!permissions || (!permissions.admin && !permissions.push)) {
        return res.status(403).send({
          message: 'Permission denied. You must have admin or write access to the repository to add it.'
        });
      }
    } catch (ghError) {
      if (ghError.response && ghError.response.status === 404) {
        return res.status(404).send({ message: 'Repository not found on GitHub (or it is private and you lack access).' });
      }
      throw ghError;
    }
...
app.delete('/api/repositories/:id', protectRoute, async (req, res) => {
  ...
  try {
    ...
    if (result.deletedCount === 0) {
      return res.status(404).send({ message: 'Repository not found or access denied.' });
    }
